...
class convert_submission extends adhoc_task {
    ...
    public function execute() {
        ...
        $conversionstillinprogress = false;
        $remainingattempts = $this->get_attempts_available() - 1;
        $delay = $this->get_soft_retry_delay();
        foreach ($users as $userid) {
            ...
            switch ($combineddocument->get_status()) {
                ...
                case combined_document::STATUS_PENDING_INPUT:
                    // Unresolved cases exist where these switch cases never resolve.
                    // Throw an exception so the task tries again.
                    mtrace("Conversion in-progress. Requeueing self to check again. Attempts remaining: {$remainingattempts}");
                    $conversionstillinprogress = true;
                    continue 2;
                case combined_document::STATUS_FAILED:
                    ...
                    mtrace("Conversion has failed. Requeueing self to try again. Attempts remaining: {$remainingattempts}.");
        ...
        if ($conversionstillinprogress) {
            if (isset($data->softretrydelay)) {
                $this->set_soft_retry_delay($data->softretrydelay);
            } else {
