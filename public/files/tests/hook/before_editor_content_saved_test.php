 ...
 */
#[CoversClass(before_editor_content_saved::class)]
#[CoversFunction('file_save_draft_area_files')]
#[CoversFunction('file_postupdate_standard_editor')]
final class before_editor_content_saved_test extends \advanced_testcase {
    ...
    public function test_readonly_properties(): void {
        ...
        $this->assertSame((int) FORMAT_HTML, $hook->format);
    }
    ...
    public function test_hook_dispatched_from_file_postupdate_standard_editor_no_files(): void {
        ...
        $this->assertSame((int) FORMAT_HTML, $receivedhook->format);
    ...
    /**
     * Test that the hook is NOT dispatched from file_postupdate_standard_editor()
     * when the editor text is null.
     */
    public function test_hook_not_dispatched_when_editor_text_is_null(): void {
        global $CFG;
        require_once($CFG->libdir . '/filelib.php');
        ...
        $this->assertFalse($dispatched, 'Hook should not be dispatched when editor text is null');
        $this->assertNull($result->description);
    }

    /**
     * Test that the hook IS dispatched from file_save_draft_area_files() even
     * when $draftitemid is IGNORE_FILE_MERGE, and that $draftitemid is passed
     * through to the hook unchanged.
     */
    public function test_hook_dispatched_with_ignore_file_merge(): void {
        global $CFG;
        require_once($CFG->libdir . '/filelib.php');
        ...
        $originaltext = '<p>Content from external client</p>';
        ...
        $this->assertSame(
            IGNORE_FILE_MERGE,
            $receivedhook->draftitemid,
            'draftitemid should be IGNORE_FILE_MERGE when called with IGNORE_FILE_MERGE'
        );
        $this->assertSame($syscontext->id, $receivedhook->contextid);
        $this->assertSame('core', $receivedhook->component);
        $this->assertSame('unittest', $receivedhook->filearea);
