 ...
 * chance to perform additional content transformations before the final value is stored.
 *
 * Typical use cases:
 ...
 *  1. {@see file_save_draft_area_files()} – when the editor has an associated draft file area and
 *     $text is not null. $draftitemid is set but $format is null.
 *     When called with IGNORE_FILE_MERGE (-1) the hook is still dispatched and $draftitemid will
 *     be IGNORE_FILE_MERGE; no file merging or URL rewriting will have occurred in that case.
 *  2. {@see file_postupdate_standard_editor()} – when no file area is used (maxfiles == 0) and
 *     $context and $text are not null. $format is set but $draftitemid is null.
 *
 * @package    core_files
...
#[\core\attribute\label('Allows plugins to transform editor content before it is saved to the database')]
