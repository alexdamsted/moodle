...
class plugin_manager {
    ...
    public function get_uninstall_url($component, $return = 'overview') {
        $reason = $this->can_plugin_be_uninstalled($component);
        if ($reason !== null) {
            return null;
        }
    ...
    protected function common_uninstall_check_reason(\core\plugininfo\base $pluginfo): ?string {
        ...
        if (!$pluginfo->is_uninstall_allowed()) {
            ...
            $reason = $pluginfo->get_uninstall_notallowed_reason();
            if ($reason) {
                return $reason;
            }
            debugging($pluginfo->type . '_' . $pluginfo->name . " didn't implement uninstall reason");
